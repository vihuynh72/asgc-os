...
const email = String(process.argv[2] ?? process.env.ASGC_PRESIDENT_EMAIL ?? "")
  .trim()
  .toLowerCase();
...
if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
  console.error("Provide a president email as the first argument or set ASGC_PRESIDENT_EMAIL in .env.local");
  process.exit(1);
}
