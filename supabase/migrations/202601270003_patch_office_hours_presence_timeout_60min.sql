-- PATCH — Office Hours presence timeout to 60 minutes (Hobby-friendly)
--
-- Motivation:
-- - Vercel Hobby cron precision is hourly, so enforcing a 15-minute "close in DB" is not reliable.
-- - Simplify the mental model: if we can't verify presence for ~1 hour, we check out.
-- - We still clamp `checkout_at` to the timeout boundary so time credit is not inflated by cron delay.

begin;

-- 1) Heartbeat RPC: close stale sessions at the 60-minute timeout boundary.
...
begin
  ...
  -- Some sessions (kiosk) are intentionally excluded from presence enforcement.
  if coalesce(s.requires_presence, true) = false then
    return query select s.id, 'ignored';
    return;
  end if;

  -- Timeout: if presence has already gone stale, close at the timeout boundary.
  timeout_at := coalesce(s.last_presence_at, s.checkin_at) + interval '60 minutes';
  if coalesce(s.last_presence_at, s.checkin_at) <= (now() - interval '60 minutes') then
...
-- 2) Cron enforcement: close sessions without heartbeat in 60 minutes (clamped).
...
begin
  ...
  for r in
    update public.office_hour_sessions as sess
    set
      checkout_at = (coalesce(sess.last_presence_at, sess.checkin_at) + interval '60 minutes'),
      status = 'closed',
      distance_m_at_checkout = null,
      needs_review = false,
      review_reason = null
    where sess.status = 'open'
      ...
      and coalesce(sess.last_presence_at, sess.checkin_at) <= (_now - interval '60 minutes')
