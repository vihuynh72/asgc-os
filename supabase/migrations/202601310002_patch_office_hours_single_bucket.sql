-- PATCH — Office Hours single-bucket policy (remove in-office vs on-behalf)
--
-- Policy:
-- - All approved minutes count toward the same weekly total.
-- - "In-office" requirement is removed (enforced as 0).
-- - RPC outputs remain compatible:
--   - in_office_minutes mirrors total_minutes
--   - deficit_in_office_minutes is always 0

begin;

-- 1) Data normalization: convert any legacy "in_office" exceptions to "total".
update public.office_hour_exceptions
set kind = 'total'
where kind = 'in_office';

-- 2) Enforce that requirements do not include an in-office component.
update public.office_hour_requirements
set weekly_in_office_hours = 0
where weekly_in_office_hours <> 0;

alter table public.office_hour_requirements
  drop constraint if exists office_hour_requirements_in_office_is_zero;

alter table public.office_hour_requirements
  add constraint office_hour_requirements_in_office_is_zero
  check (weekly_in_office_hours = 0);

-- 3) Enforce exceptions kind is only "total".
alter table public.office_hour_exceptions
  drop constraint if exists office_hour_exceptions_kind_check;

alter table public.office_hour_exceptions
  add constraint office_hour_exceptions_kind_check
  check (kind in ('total'));

-- 4) Weekly rollup RPC (member): treat all minutes equally.
...
-- 5) Weekly rollup RPC (admin): treat all minutes equally.
...
-- 6) Weekly reminder enqueue: only total deficits.
