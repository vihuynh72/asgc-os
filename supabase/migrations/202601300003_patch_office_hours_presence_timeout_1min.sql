-- PATCH — Office Hours strict presence timeout to 1 minute + lightweight ping
--
-- Behavior:
-- - Client can keep session alive without constant geolocation via `record_office_hours_presence_ping()`.
-- - Geolocation heartbeat still enforces geofence via `record_office_hours_presence(lat, lon)`.
-- - Cron/backstop checks out when `last_presence_at` is stale for >= 1 minute (clamped).
...
begin
  ...
  timeout_at := coalesce(s.last_presence_at, s.checkin_at) + interval '1 minute';
  if coalesce(s.last_presence_at, s.checkin_at) <= (now() - interval '1 minute') then
...
-- 2) Geolocation heartbeat: stale check + geofence enforcement + update last_presence_at.
...
-- 3) Cron enforcement: close sessions without presence in 1 minute (clamped).
...
begin
  ...
  for r in
    update public.office_hour_sessions as sess
    set
      checkout_at = (coalesce(sess.last_presence_at, sess.checkin_at) + interval '1 minute'),
      status = 'closed',
      distance_m_at_checkout = null,
      needs_review = false,
      review_reason = null
    where sess.status = 'open'
      ...
      and coalesce(sess.last_presence_at, sess.checkin_at) <= (_now - interval '1 minute')
