-- PATCH — Office Hours credit policy: manual checkout required
--
-- Rule:
-- - Only sessions with status='closed' count toward weekly totals.
-- - Sessions with status='auto_closed' do NOT count (e.g., auto-close, auto-checkout).
--
-- Admins can still grant credit via office_hour_exceptions.

begin;

-- Member weekly rollup: closed-only.
...
-- Admin weekly rollup: closed-only.
...
-- Weekly reminder candidates: use credited (closed-only) totals.
-- Drop first to allow return type change (column names differ from prior version).
