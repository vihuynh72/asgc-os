-- FIX — Include auto_closed sessions in weekly hour totals
--
-- Migration 202603230001 correctly changed auto-checkout to set
-- status = 'auto_closed' instead of 'closed', but the weekly hours
-- functions still only counted status = 'closed'. This caused all
-- auto-closed sessions (geofence departure, 5pm timeout, cron) to
-- receive zero credit in member/admin dashboards and reminders.
--
-- This migration updates my_weekly_hours(), admin_weekly_hours(), and
-- enqueue_weekly_hours_reminders() to include auto_closed sessions.

begin;

-- 1) Member weekly rollup: include auto_closed sessions.
...
-- 2) Admin weekly rollup: include auto_closed sessions.
...
-- 3) Weekly reminder candidates: include auto_closed sessions.
