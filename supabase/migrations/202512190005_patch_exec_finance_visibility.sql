-- PATCH — Executive finance visibility + docs roles alignment
-- Source of truth: 02_data_model.md, 03_security_and_permissions.md

begin;

-- 1) Role helpers for executive/board/finance access.
...
create or replace function public.is_board_member(_uid uuid)
returns boolean
language plpgsql
stable
security definer
...
begin
  ...
  return public.is_admin(_uid) or public.is_executive(_uid);
end;
$$;

revoke all on function public.is_finance_admin(uuid) from public;
grant execute on function public.is_finance_admin(uuid) to authenticated;

-- 2) Expand docs types for finance/grants exports.
...
-- 3) Update doc visibility helper to include executives for restricted docs.
...
begin
  ...
  case doc_row.visibility
    ...
    when 'restricted' then
      return public.is_executive(auth.uid());
...
-- 4) Update docs restricted policy to include executives.
drop policy if exists "docs_select_restricted" on public.docs;
...
create policy "docs_select_restricted"
  ...
  using (
    ...
    and not is_deleted
    and (public.is_admin(auth.uid()) or public.is_executive(auth.uid()))
  );

-- 5) Update create_doc to allow executives to set restricted docs.
...
-- 6) Update update_doc to allow executives to set restricted docs.
