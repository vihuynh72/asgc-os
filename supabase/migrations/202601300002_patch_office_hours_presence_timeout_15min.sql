-- PATCH — Office Hours strict presence timeout to 15 minutes
--
-- Expected behavior:
-- - If a user closes their tab and we stop receiving heartbeats, the session is auto-closed once
--   `last_presence_at` is older than 15 minutes.
-- - We clamp `checkout_at` to the timeout boundary so time credit is not inflated by cron delay.

begin;

-- 1) Heartbeat RPC: close stale sessions at the 15-minute timeout boundary.
...
-- 2) Cron enforcement: close sessions without heartbeat in 15 minutes (clamped).
